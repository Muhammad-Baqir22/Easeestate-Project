  ...
  return (
    <>
        ...
        <div className="relative z-10 flex flex-col items-center">
          ...
          <div className="bg-white/95 backdrop-blur-lg rounded-3xl shadow-2xl p-8 w-full max-w-[640px] border border-white/40">
              ...
              {['buy', 'rent'].map(m => (
                <button key={m} onClick={() => setMode(m)}
                  className={`flex-1 py-2.5 rounded-[10px] text-[15px] font-bold transition-all duration-200 capitalize ${
                    mode === m ? 'bg-navy text-white shadow-sm' : 'text-muted hover:text-navy hover:bg-navy/6'
                  }`}
                  style={{ fontFamily: 'var(--font-heading)' }}>
                  {m === 'buy' ? 'Buy' : 'Rent'}
                </button>
            ...
            <form onSubmit={handleSearch} className="flex gap-3 items-stretch">
              <input type="text" placeholder="Enter city or location..." value={location}
                onChange={e => setLocation(e.target.value)} required
                className="flex-1 px-4 py-3.5 border border-border-c rounded-xl text-[15px] text-ink bg-white outline-none focus:border-navy-mid focus:ring-2 focus:ring-navy-mid/10 min-w-0" />
              <select value={size} onChange={e => setSize(e.target.value)}
                className="px-3 py-3.5 border border-border-c rounded-xl text-[15px] text-slate bg-white outline-none focus:border-navy-mid cursor-pointer">
              ...
              <button type="submit" className="btn px-6 flex-shrink-0">Search</button>
            </form>
          </div>
      ...
      <section className="py-20 px-6 bg-surface text-center">
        ...
        <h2 className="text-3xl font-bold text-navy mb-2" style={{ fontFamily: 'var(--font-heading)' }}>Featured Colonies</h2>
        <p className="text-muted text-[15px] mb-10 max-w-[400px] mx-auto">Pakistan&apos;s most sought-after residential areas</p>
      ...
      <section className="py-16 px-6 bg-navy text-center">
        ...
        <p className="text-white/60 mb-8 max-w-[400px] mx-auto">
          Join thousands of sellers and reach millions of buyers across Pakistan.
        </p>
        <div className="flex gap-4 justify-center">
          <Link to="/signup"><button className="btn px-8 py-3 text-[15px]">Get Started Free</button></Link>
          <Link to="/login">
            <button className="px-8 py-3 border border-white/40 text-white/80 rounded-xl text-[15px] font-semibold hover:border-gold-bright hover:text-gold-bright transition-all"
              style={{ fontFamily: 'var(--font-heading)' }}>
              Log In
            </button>
          </Link>
