    ...
    <>
      ...
      <main className="max-w-[720px] mx-auto px-6 py-12">
        <div className="mb-8">
          ...
          <p className="text-muted mt-1.5">Fill in the details below to list your rental property.</p>
        </div>

        <form onSubmit={handleSubmit} className="bg-white rounded-2xl border border-border-c shadow-sm p-9 flex flex-col gap-5">
          <Field label="Property Title">
            <input type="text" placeholder="e.g. 5 Marla House for Rent in Gulberg" value={form.title}
          ...
          <div className="grid grid-cols-2 gap-4">
            <Field label="City / Location">
              <input type="text" placeholder="e.g. Islamabad" value={form.location}
                onChange={set('location')} required className="field" />
            </Field>
            <Field label="Colony / Area">
              <input type="text" placeholder="e.g. F-7" value={form.colony}
                onChange={set('colony')} required className="field" />
            </Field>
          </div>

          <Field label="Monthly Rent (Rs)">
            <input type="number" placeholder="Enter monthly rent" value={form.price}
          ...
          <button type="submit" className="btn w-full py-4 text-[15px] mt-2" disabled={submitting}>
            {submitting ? 'Publishing...' : 'Publish Rental Listing'}
          </button>
        </form>
      </main>
      <Footer />
